...
class STREEBOG(PEP247):
    ...
    def copy(self):
        return STREEBOG(copy(self.data), self.digest_size)
    ...
    def full_update(self, data=b""):
        """Change data that has to be hashed in one variable"""
        self.data = data.encode('utf-8')
    ...
    def count_iterations(self, num_of_iterations):
        for i in range(num_of_iterations):
            self.full_update(self.hexdigest())
        return self

class Streebog_PasswordHasher(BasePasswordHasher):

    algorithm = "gost34_11_2012"
    ...
    def encode(self, password, salt):
        ...
        hsh = STREEBOG(digest_size=64)
        hsh.update(salt+password)
        hash = hsh.hexdigest()
